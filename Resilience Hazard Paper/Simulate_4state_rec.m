function [RESULT] = Simulate_4state_rec(IM, T, dt, Nsims)
...
% if strcmp(Hazard,'E') == 1
    ...
    [tot_time] = Rep_dists_4States(tr, IM);
% else
%     
%     [tot_time] = Rep_dists_Li_Ell_Hurr_Char_SMPRESS(tr, IM);
% end
...
%     if isnan(round(Times(ii,1)))==1 || isnan(round(Times(ii,1)+Times(ii,2)))==1 || isnan(round(Times(ii,1)+Times(ii,2)+Times(ii,3)))==1
%         ii=ii-1;
%         continue
%     end
...
%     Times(ii,4) = 0;
    ...
    States(ii,ind1:ind2) = 0.333;
    States(ii,(ind2+1):ind3) = 0.667;
    States(ii,(ind3+1):length(tr)) = 1;
